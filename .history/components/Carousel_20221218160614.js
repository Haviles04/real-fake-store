...
function Carousel({ products }) {
  ...
  const scrollBackwards = () => {
    ...
    if (!caro.current) return;
      ...
      if ((currentX.current = offset)) {
        currentX.current = 0;
      } else if (currentX.current >= carouselWidth - 5) {
        currentX.current = offset * 2;
      } else currentX.current = currentX.current - offset;
      caro.current.scroll({ left: `${currentX.current}`, behavior: "smooth" });
    }
  };
  ...
  return (
    <div className={styles.main}>
      ...
      <div ref={caro} className={styles.mainCaro} style={{width: `${pcRef.current.offsetWidth / 3}`}}>
        {products.map((item) => (
