...
function Carousel({ products }) {
  ...
  console.log(pcRef.current.offsetWidth);
  ...
  const scrollBackwards = () => {
    ...
    if (!caro.current) return;
      ...
      if (currentX.current > carouselWidth ) {
        currentX.current = offset * 2;
      } else if(currentX.current = offset  ){
        console.log(currentX.current)
        currentX.current = 0;
      }
      
      
      if(currentX.current !== 0 ){currentX.current = currentX.current - offset;}
      caro.current.scroll({ left: `${currentX.current}`, behavior: "smooth" });
    }
  };
