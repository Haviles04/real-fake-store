...
function Carousel({ products }) {
  ...
  const showOrHideBtns = () => {
    if (
      swipeDirection === "Left" &&
      newX.current >=
        caro.current.scrollWidth - offset - pcRef.current.offsetWidth
    ) {
      setMaxForward(true);
    } else if (swipeDirection === "Right" && newX.current >= 0) {
      setMaxBackwards(false);
    } else if (
      onMobile.current &&
      newX.current >= caro.current.scrollWidth - offset
    ) {
      setMaxForward(true);
    } else if (newX.current >= caro.current.scrollWidth - offset - 1) {
      setMaxForward(true);
    } else if (newX.current <= pcRef.current.offsetWidth + 1) {
