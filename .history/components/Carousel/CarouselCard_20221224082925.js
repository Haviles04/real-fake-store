...
function CarouselCard({ item, pcRef, currentX,setMaxBackward,setMaxForward, caro, products }) {
  ...
  const handleTouch = (e) => {
   ...
   currentX.current = e.offsetLeft;
    if (onMobile && currentX.current >= pcRef.current.offsetWidth * (products.length - 1)) {
  ...
  return (
    <div
      ...
      key={item.id}
      onTouchEnd={(e) => {handleTouch(e.currentTarget)}}
    >
