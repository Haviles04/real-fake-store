...
function Carousel({ products }) {
  ...
  const scrollBackwards = () => {
    ...
    let offset = pcRef.current.offsetWidth * 3;
      currentX.current = currentX.current - offset;
      caro.current.scroll({ left: `${currentX.current}`, behavior: "smooth" });
      currentX.current === 0 ? setMaxBackwards(true) : setMaxBackwards(false);
  };
  ...
  return (
    <div className={styles.main}>
      <button onClick={() => scrollBackwards()} className={styles.buttons} style={maxBackwards ? {display:"none"} : {display: "flex"}}>
