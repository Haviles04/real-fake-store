...
function Carousel({ products }) {
  ...
  const scrollForward = () => {
    ...
    onMobile ? offset = pcRef.current.offsetWidth * 2: offset = pcRef.current.offsetWidth * 3;
  ...
  const scrollBackwards = () => {
    setMaxForward(false);
    caro.current.offsetWidth >= 510 ? offset = pcRef.current.offsetWidth * 2: offset = pcRef.current.offsetWidth * 3;
      currentX.current = currentX.current - offset;
      caro.current.scroll({ left: `${currentX.current}`, behavior: "smooth" });
