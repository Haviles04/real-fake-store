...
function Carousel({ products }) {
  ...
  const showOrHideBtns = () => {
    if(onMobile && currentX.current >= (caro.current.scrollWidth - offset) - pcRef.current.offsetWidth){
      setMaxForward(true)
    }else if(currentX.current >= caro.current.scrollWidth + pcRef.offsetWidth){
      setMaxForward(true);
  ...
  const handleTouchEnd = (e) => {
    ...
    setSwipeDirection(currentX.current, touchEndX);
    checkOnMobile();
    showOrHideBtns();
  };
